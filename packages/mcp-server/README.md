...
> [!NOTE]
> Once this package is [published to npm](https://www.stainless.com/docs/guides/publish), this will become: `npx -y morta-mcp`
...
```json
{
  "mcpServers": {
    "morta_api": {
      "command": "node",
      "args": ["/path/to/local/morta-typescript/packages/mcp-server", "--client=claude", "--tools=dynamic"],
      "env": {
