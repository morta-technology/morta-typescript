...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['comment_thread_id', 'comment_id', 'commentText'],
    $defs: {
...
export const handler = async (client: Morta, args: Record<string, unknown> | undefined) => {
  const { comment_id, jq_filter, ...body } = args as any;
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.commentThread.comment.update(comment_id, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
