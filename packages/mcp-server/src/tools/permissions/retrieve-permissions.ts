...
export const handler = async (client: Morta, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.permissions.retrieve(body)));
  } catch (error) {
    if (isJqError(error)) {
