...
export const handler = async (client: Morta, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.document.section.response.reset(document_response_id, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
