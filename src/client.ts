...
import {
  IntegrationCreatePassthroughDownloadParams,
  IntegrationCreatePassthroughParams,
...
} from './resources/comment-thread/comment-thread';
import {
  Document,
  DocumentCreateMultipleSectionsParams,
  DocumentCreateMultipleSectionsResponse,
  DocumentCreateParams,
  DocumentCreateResponse,
  DocumentCreateSectionsParams,
  DocumentCreateSectionsResponse,
  DocumentDeleteResponse,
  DocumentExportParams,
  DocumentGetDeletedSectionsParams,
...
import {
  ...
  TableStreamRowsParams,
  TableTruncateResponse,
  TableUpdateCellsParams,
  TableUpdateCellsResponse,
  TableUpdateParams,
  TableUpdateResponse,
  UpdateDocumentTableCells,
} from './resources/table/table';
import {
  Event,
  SummaryUser,
  User,
...
export interface ClientOptions {
  /**
   * The Morta API uses API Keys to authenicate requests. You can view and manage your API keys in the profile section of your user account.Your API keys carry permissions tied to your user account, so please make sure to keep them secure. Avoid storing your API keys in your version control system.All API requests are made over HTTPS, Morta doesn't support plain HTTP connections.
   */
   ...
   *
   * Defaults to process.env['MORTA_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['MORTA_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Morta API.
 */
export class Morta {
  apiKey: string;
   ...
   * API Client for interfacing with the Morta API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['MORTA_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['MORTA_BASE_URL'] ?? https://api.morta.io] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('MORTA_BASE_URL'),
    apiKey = readEnv('MORTA_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.MortaError(
        "The MORTA_API_KEY environment variable is missing or empty; either provide it, or instantiate the Morta client with an apiKey option, like new Morta({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.morta.io`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Morta.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('MORTA_LOG'), "process.env['MORTA_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('MORTA_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.morta.io';
  }
  ...
  static MortaError = Errors.MortaError;
  ...
  hub: API.Hub = new API.Hub(this);
  table: API.TableResource = new API.TableResource(this);
  document: API.DocumentResource = new API.DocumentResource(this);
  notifications: API.Notifications = new API.Notifications(this);
  commentThread: API.CommentThreadResource = new API.CommentThreadResource(this);
...
Morta.DocumentResource = DocumentResource;
Morta.Notifications = Notifications;
...
export declare namespace Morta {
  ...
  export {
    CommentThreadResource as CommentThreadResource,
